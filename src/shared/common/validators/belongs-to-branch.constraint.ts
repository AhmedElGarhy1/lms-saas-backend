...
@Injectable()
export class BelongsToBranchConstraint implements ValidatorConstraintInterface {
  ...
  async validate(value: any, args: ValidationArguments): Promise<boolean> {
    ...
    if (!this.dataSource) {
      console.error('DataSource is not available in BelongsToBranchConstraint');
      return false;
    }
    ...
    try {
      ...
      return (branch as { centerId?: string }).centerId === centerId;
    } catch (error) {
      console.error('Error in BelongsToBranchConstraint:', error);
      return false;
    }
  }
