...
/**
 * Resolver for CENTER_ACCESS_ACTIVATED notification intent
 * Handles TARGET and OWNERS audiences
 */
@Injectable()
export class CenterAccessActivatedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.CENTER_ACCESS_ACTIVATED>
{
  private readonly logger: Logger = new Logger(CenterAccessActivatedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.CENTER_ACCESS_ACTIVATED>,
    audience: AudienceIdForNotification<NotificationType.CENTER_ACCESS_ACTIVATED>,
  ) {
    ...
    if (!userProfile) {
      throw new Error(
        `CENTER_ACCESS_ACTIVATED: UserProfile not found: ${intent.userProfileId}`,
      );
    }
    ...
    if (!center) {
      throw new Error(
        `CENTER_ACCESS_ACTIVATED: Center not found: ${intent.centerId}`,
      );
    }
    ...
    if (!user) {
      throw new Error(
        `CENTER_ACCESS_ACTIVATED: User not found: ${userProfile.userId}`,
      );
    }
    ...
    if (!actor) {
      throw new Error(
        `CENTER_ACCESS_ACTIVATED: Actor profile not found: ${intent.actorId}`,
      );
    }
    ...
    if (!actorUser) {
      throw new Error(
        `CENTER_ACCESS_ACTIVATED: Actor user not found: ${actor.userId}`,
      );
    }
