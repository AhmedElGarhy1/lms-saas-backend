...
/**
 * Resolver for CENTER_ACCESS_REVOKED notification intent
 * Handles TARGET and OWNERS audiences
 */
@Injectable()
export class CenterAccessRevokedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.CENTER_ACCESS_REVOKED>
{
  private readonly logger: Logger = new Logger(CenterAccessRevokedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.CENTER_ACCESS_REVOKED>,
    audience: AudienceIdForNotification<NotificationType.CENTER_ACCESS_REVOKED>,
  ) {
    ...
    if (!userProfile) {
      throw new Error(
        `CENTER_ACCESS_REVOKED: UserProfile not found: ${intent.userProfileId}`,
      );
    }
    ...
    if (!center) {
      throw new Error(
        `CENTER_ACCESS_REVOKED: Center not found: ${intent.centerId}`,
      );
    }
    ...
    if (!user) {
      throw new Error(
        `CENTER_ACCESS_REVOKED: User not found: ${userProfile.userId}`,
      );
    }
    ...
    if (!actor) {
      throw new Error(
        `CENTER_ACCESS_REVOKED: Actor profile not found: ${intent.actorId}`,
      );
    }
    ...
    if (!actorUser) {
      throw new Error(
        `CENTER_ACCESS_REVOKED: Actor user not found: ${actor.userId}`,
      );
    }

    // Get profile type in Arabic
    ...
    const profileType = profileTypeMap[userProfile.profileType] || 'مستخدم';
