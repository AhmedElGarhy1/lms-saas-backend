...
/**
 * Resolver for ROLE_ASSIGNED notification intent
 * Handles TARGET and OWNERS audiences
 */
@Injectable()
export class RoleAssignedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.ROLE_ASSIGNED>
{
  private readonly logger: Logger = new Logger(RoleAssignedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.ROLE_ASSIGNED>,
    audience: AudienceIdForNotification<NotificationType.ROLE_ASSIGNED>,
  ) {
    ...
    if (!userProfile) {
      throw new Error(
        `ROLE_ASSIGNED: UserProfile not found: ${intent.userProfileId}`,
      );
    }
    ...
    if (!role) {
      throw new Error(`ROLE_ASSIGNED: Role not found: ${intent.roleId}`);
    }

    const user = await this.userService.findOne(userProfile.userId);
    if (!user) {
      throw new Error(`ROLE_ASSIGNED: User not found: ${userProfile.userId}`);
    }

    const actor = await this.userProfileService.findOne(intent.actorId);
    if (!actor) {
      throw new Error(
        `ROLE_ASSIGNED: Actor profile not found: ${intent.actorId}`,
      );
    }

    const actorUser = await this.userService.findOne(actor.userId);
    if (!actorUser) {
      throw new Error(`ROLE_ASSIGNED: Actor user not found: ${actor.userId}`);
    }
