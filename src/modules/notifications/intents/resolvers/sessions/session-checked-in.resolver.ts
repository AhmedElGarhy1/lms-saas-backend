...
/**
 * Resolver for SESSION_CHECKED_IN notification intent
 *
 ...
 */
@Injectable()
export class SessionCheckedInResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.SESSION_CHECKED_IN>
{
  private readonly logger: Logger = new Logger(SessionCheckedInResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.SESSION_CHECKED_IN>,
    audience: AudienceIdForNotification<NotificationType.SESSION_CHECKED_IN>,
  ) {
    const session = await this.sessionsRepository.findById(intent.sessionId);
    if (!session) {
      throw new Error(
        `SESSION_CHECKED_IN: Session not found: ${intent.sessionId}`,
      );
    }
    ...
    if (!group) {
      throw new Error(`SESSION_CHECKED_IN: Group not found: ${intent.groupId}`);
    }

    const center = await this.centersRepository.findOne(intent.centerId);
    if (!center) {
      throw new Error(
        `SESSION_CHECKED_IN: Center not found: ${intent.centerId}`,
      );
    }
    ...
    const templateVariables = {
      ...
      startTime: format(session.actualStartTime || session.startTime, 'h:mm a', {
        locale: ar,
      }),
      teacherName,
    ...
    switch (audience) {
      ...
      case 'PARENTS':
        ...
        break;
      // TEACHER: No notification - they performed the action
      case 'STAFF':
