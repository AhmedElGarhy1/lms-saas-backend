...
/**
 * Resolver for USER_PROFILE_ACTIVATED notification
 * TARGET only. Push + In-App.
 */
@Injectable()
export class UserProfileActivatedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.USER_PROFILE_ACTIVATED>
{
  private readonly logger = new Logger(UserProfileActivatedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.USER_PROFILE_ACTIVATED>,
    audience: AudienceIdForNotification<NotificationType.USER_PROFILE_ACTIVATED>,
  ) {
    ...
    if (!profile) {
      throw new Error(
        `USER_PROFILE_ACTIVATED: UserProfile not found: ${intent.userProfileId}`,
      );
    }
    ...
    if (!user) {
      throw new Error(
        `USER_PROFILE_ACTIVATED: User not found: ${profile.userId}`,
      );
    }
