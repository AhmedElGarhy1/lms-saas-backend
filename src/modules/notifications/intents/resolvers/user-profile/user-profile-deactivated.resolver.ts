...
/**
 * Resolver for USER_PROFILE_DEACTIVATED notification
 * TARGET only. Push + In-App.
 */
@Injectable()
export class UserProfileDeactivatedResolver
  extends BaseIntentResolver
  implements
    NotificationIntentResolver<NotificationType.USER_PROFILE_DEACTIVATED>
{
  private readonly logger = new Logger(UserProfileDeactivatedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.USER_PROFILE_DEACTIVATED>,
    audience: AudienceIdForNotification<NotificationType.USER_PROFILE_DEACTIVATED>,
  ) {
    ...
    if (!profile) {
      throw new Error(
        `USER_PROFILE_DEACTIVATED: UserProfile not found: ${intent.userProfileId}`,
      );
    }
    ...
    if (!user) {
      throw new Error(
        `USER_PROFILE_DEACTIVATED: User not found: ${profile.userId}`,
      );
    }
