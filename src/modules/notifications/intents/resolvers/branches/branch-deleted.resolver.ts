...
/**
 * Resolver for BRANCH_DELETED notification intent
 * Branch is soft-deleted; use findOneSoftDeletedById.
 * Handles OWNERS and STAFF audiences.
 */
@Injectable()
export class BranchDeletedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.BRANCH_DELETED>
{
  private readonly logger: Logger = new Logger(BranchDeletedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.BRANCH_DELETED>,
    audience: AudienceIdForNotification<NotificationType.BRANCH_DELETED>,
  ) {
    const branch =
      await this.branchesRepository.findOneSoftDeletedById(intent.branchId);
    ...
    if (!center) {
      throw new Error(
        `BRANCH_DELETED: Center not found: ${intent.centerId}`,
      );
    }
