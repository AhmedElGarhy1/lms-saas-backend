...
/**
 * Resolver for BRANCH_RESTORED notification intent
 * Handles OWNERS and STAFF audiences.
 */
@Injectable()
export class BranchRestoredResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.BRANCH_RESTORED>
{
  private readonly logger: Logger = new Logger(BranchRestoredResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.BRANCH_RESTORED>,
    audience: AudienceIdForNotification<NotificationType.BRANCH_RESTORED>,
  ) {
    const branch = await this.branchesRepository.findOne(intent.branchId);
    if (!branch) {
      throw new Error(
        `BRANCH_RESTORED: Branch not found: ${intent.branchId}`,
      );
    }
    const center = await this.centersRepository.findOne(intent.centerId);
    if (!center) {
      throw new Error(
        `BRANCH_RESTORED: Center not found: ${intent.centerId}`,
      );
    }
