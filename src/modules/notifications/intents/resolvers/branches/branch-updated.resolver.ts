...
/**
 * Resolver for BRANCH_UPDATED notification intent
 * Handles OWNERS and STAFF audiences.
 */
@Injectable()
export class BranchUpdatedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.BRANCH_UPDATED>
{
  private readonly logger: Logger = new Logger(BranchUpdatedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.BRANCH_UPDATED>,
    audience: AudienceIdForNotification<NotificationType.BRANCH_UPDATED>,
  ) {
    const branch = await this.branchesRepository.findOne(intent.branchId);
    if (!branch) {
      throw new Error(
        `BRANCH_UPDATED: Branch not found: ${intent.branchId}`,
      );
    }
    const center = await this.centersRepository.findOne(intent.centerId);
    if (!center) {
      throw new Error(
        `BRANCH_UPDATED: Center not found: ${intent.centerId}`,
      );
    }
