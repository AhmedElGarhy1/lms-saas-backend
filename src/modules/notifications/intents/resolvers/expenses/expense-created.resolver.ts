...
/**
 * Resolver for EXPENSE_CREATED notification
 * OWNERS + STAFF (branch). In-App only.
 */
@Injectable()
export class ExpenseCreatedResolver
  extends BaseIntentResolver
  implements NotificationIntentResolver<NotificationType.EXPENSE_CREATED>
{
  private readonly logger = new Logger(ExpenseCreatedResolver.name);
  ...
  async resolveIntent(
    intent: IntentForNotification<NotificationType.EXPENSE_CREATED>,
    audience: AudienceIdForNotification<NotificationType.EXPENSE_CREATED>,
  ) {
    ...
    if (!expense) {
      throw new Error(
        `EXPENSE_CREATED: Expense not found: ${intent.expenseId}`,
      );
    }
    ...
    if (!center) {
      throw new Error(
        `EXPENSE_CREATED: Center not found for expense: ${intent.expenseId}`,
      );
    }
